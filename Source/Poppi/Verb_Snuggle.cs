...
public class Verb_Snuggle : Verb_MeleeAttack
{
    protected override DamageWorker.DamageResult ApplyMeleeDamageToTarget(LocalTargetInfo target)
    {
        ...
        var damageInfo = new DamageInfo(RimWorld.DamageDefOf.Scratch, (int)tool.power, 0f, -1f, base.CasterPawn);
        damageResult.hitThing = target.Thing;
        damageResult.totalDamageDealt = Mathf.Min(target.Thing.HitPoints, damageInfo.Amount);
        for (var i = 0; i < 8; i++)
        {
            ...
            pawn.TakeDamage(new DamageInfo(DamageDefOf.Poppi_Cleave, (int)(tool.power * 0.6f), 0f, -1f,
                base.CasterPawn));
